...
func fetchNewsSources() bool {
	...
	for _, newsSource := range newsSourcesResp.Sources {
		...
		if newsSource.Id == "reddit-r-all" {
			newsSource.Category = "entertainment"
		}
...
//////////////////////////////////////////////////////////////////////////////
//
// report news article stats - such as # articles, from the DB, so we can monitor
//     the progress of this service.
//
//////////////////////////////////////////////////////////////////////////////
...
func NewsService() {
	...
	for {
		...
		if flags.offlineNews != "" {
			...
			// Fetch news sources.  (Necessary for mapping news source to category to infer category.)
			...
			// TODO: Fetch headlines for the US: https://newsapi.org/v1/top-headlines?country=us
			//pr("Fetching headlines")
			//go fetchNews("")
			//return;
				...
				if len(newArticles) > 0 {
					...
					for _, a := range newArticles {
						...
						if len(publishedAt) == 0 {
							publishedAt = time.Now().Format(time.UnixDate) // << THIS IS THE FIX!  (Sources like "national-geographic" are missing publishedAt date.
								//"epoch" //"1970-01-01 00:00:00" - January 1, year 1 00:00:00 UTC. << THIS CAUSES A SORTING ISSUE!
						}
